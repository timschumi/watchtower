Watchtower is itself packaged as a Docker container so installation is as simple as pulling the `timschumi/watchtower` image. If you are using ARM based architecture, pull the appropriate `timschumi/watchtower:armhf-<tag>` image from the [containrrr Docker Hub](https://hub.docker.com/r/timschumi/watchtower/tags/).
...
docker run -d \
  ...
  -v /var/run/docker.sock:/var/run/docker.sock \
  timschumi/watchtower container_to_watch --debug
```
