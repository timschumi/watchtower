...
var _ = Describe("the auth module", func() {
	...
	Describe("GetAuthURL", func() {
		It("should create a valid auth url object based on the challenge header supplied", func() {
			...
			imageRef, err := ref.ParseNormalizedNamed("timschumi/watchtower")
			Expect(err).NotTo(HaveOccurred())
			expected := &url.URL{
				Host:     "ghcr.io",
				Scheme:   "https",
		...
		When("deriving the auth scope from an image name", func() {
			...
			It("should not include vanity hosts\"", func() {
				Expect(getScopeFromImageAuthURL("docker.io/timschumi/watchtower")).To(Equal("timschumi/watchtower"))
				Expect(getScopeFromImageAuthURL("index.docker.io/timschumi/watchtower")).To(Equal("timschumi/watchtower"))
			})
			It("should not destroy three segment image names\"", func() {
				Expect(getScopeFromImageAuthURL("piksel/timschumi/watchtower")).To(Equal("piksel/timschumi/watchtower"))
				Expect(getScopeFromImageAuthURL("ghcr.io/piksel/timschumi/watchtower")).To(Equal("piksel/timschumi/watchtower"))
			})
			It("should not prepend library/ to image names if they're not on dockerhub", func() {
				...
				Expect(getScopeFromImageAuthURL("ghcr.io/timschumi/watchtower")).To(Equal("timschumi/watchtower"))
			})
		})
	...
	Describe("GetChallengeURL", func() {
		It("should create a valid challenge url object based on the image ref supplied", func() {
			...
			imageRef, _ := ref.ParseNormalizedNamed("ghcr.io/timschumi/watchtower:latest")
		...
		It("should assume Docker Hub for image refs with no explicit registry", func() {
			...
			imageRef, _ := ref.ParseNormalizedNamed("timschumi/watchtower:latest")
			Expect(auth.GetChallengeURL(imageRef)).To(Equal(expected))
		})
		It("should use index.docker.io if the image ref specifies docker.io", func() {
			expected := url.URL{Host: "index.docker.io", Scheme: "https", Path: "/v2/"}
			imageRef, _ := ref.ParseNormalizedNamed("docker.io/timschumi/watchtower:latest")
			Expect(auth.GetChallengeURL(imageRef)).To(Equal(expected))
		})
	})
})
